...
import ElTree from 'yak-ui/components/tree'
  ...
  computed: {
    suffixIconClass() {
      ...
      const criteria =
        ...
        this.value !== ''
      if (criteria) {
        classes = [...classes, 'el-icon-close-circle', 'is-show-close']
      } else {
        classes.push('el-icon-down')
        if (this.visible) {
    ...
    setSelected() {
      if (this.multiple) {
        ...
        if (this.showCheckbox) {
          this.$refs.tree && this.$refs.tree.setCheckedNodes(result)
        }
